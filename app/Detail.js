...
class Detail extends React.Component {
	...
	onWebViewMessage = (event: WebViewMessageEvent) => {
	  ...
	  onWebViewMessageTow = (event: WebViewMessageEvent) => {
		...
		this.setState({webViewHeightTow: (Number(event.nativeEvent.data) + 10)})
	  }
  render() {
    ...
    return (
      <View style={[styles.wrapper,{flex: 1}]}>
          {
        (!this.state.progess) ?
              <Logo navigation={this.props.navigation} length={this.state.cart}/> : null
         ...
         source={{ uri: 'http://103.237.144.246/api/get-product-slider/'+this.state.product.id }} />
			  ...
			  
			  {
									this.state.size.map((val, index_) => {
										...
										return (
						<TouchableOpacity style={{paddingBottom: 5, paddingTop:5, backgroundColor: '#f7f7f7', marginLeft: 15, width:30, borderWidth: 1}}>
						<Text style={[styles.normal_i_title,{width: 30, textAlign: 'center'}]}>{val}</Text>
						
						</TouchableOpacity>
					)
										}else{
											return (
						<TouchableOpacity style={{paddingBottom: 5, paddingTop:5, backgroundColor: '#f7f7f7', marginLeft: 15, width:30}} onPress={() => {
							let stra = val;
			stra = stra.toLowerCase();
        stra = stra.replace(/à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ/g,"a"); 
        stra = stra.replace(/è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ/g,"e"); 
